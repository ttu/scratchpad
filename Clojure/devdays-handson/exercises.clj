...
;;
;; (= 4 _)
(second (reverse my-seq))
((comp second reverse) my-seq)

(defn secToLast [x]
  ((comp second reverse)x))
...
(def asNumbers (map as-int numbers-in-string))

(max 1 2 3 4)
(max [1 2 3 4])
(apply max [1 2 3 4])

(partition 5 1 asNumbers)


(map #(reduce * %) (partition 5 1 asNumbers))

(apply max(map #(reduce * %) (partition 5 1 asNumbers)))
